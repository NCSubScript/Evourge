    ...
    def move(self, collective = True):
        ...
        if len(self.sizeChanges):
            while len(self.sizeChanges):
                ...
                self.sizes[item["from"]].pop('key', item["id"])
                print(f'{creature=}')
...
class Creature:
    ...
    def render(self, display):
        ...
        #     moved = False
                ...
                # self.location[0] += self.moveToStep[0]
                # self.location[1] += self.moveToStep[0]
                # creature.location[0] += creature.moveToStep[0]
                # creature.location[1] += creature.moveToStep[0]
                # moved = True
            # if self.location[0] < creature.location[0] 
            #     self.location[0] += self.moveToStep[0]
            #     self.location[1] += self.moveToStep[0]

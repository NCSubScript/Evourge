...
class Viewport(Entity):
    ...
    def setSize(self, size = None):
        ...
        if self.rect is not None:
            self.rect.update(int(self.rect.center[0] - ((self.width * self.zoom) / 2)), int(self.rect.center[1] - ((self.height * self.zoom) / 2)), int(self.width * self.zoom), int(self.height * self.zoom))
...
class Minimap(GroupSingle):
    ...
    def render(self, display):
        if self.app.now - self.lastUpdate >= 1:
    ...
    def processLeftClick(self, pos):
        if self.fraction == 1:
            self.app.gui.window.children["field"].moveViewportTo(((pos[0] - self.sprite.rect.left) * self.scaleUp, (pos[1] - self.sprite.rect.top) * self.scaleUp))
        else:
            self.app.gui.window.children["field"].moveViewportTo((pos[0] * self.scaleUp, (pos[1] - self.sprite.rect.top) * self.scaleUp))
...
class Field(GroupSingle):
    ...
    def render(self, display):
        ...
        bgimage = transform.scale(bgimage, (self.viewport.sprite.rect.width, self.viewport.sprite.rect.height))
        renderSurface.blit(bgimage, (0,0))
